...
const Modal = ({ isModalInfo, toggleModalInfo }) => {
    ...
    return (
        <div className={`${isModalInfo ? 'block' : 'hidden'} absolute z-20 w-full text-sm transition-all `}>
            <div className="bg-white p-3 h-screen overflow-y-auto">
                <h1 className="text-center w-full pb-5 font-bold text-lg">Upload (i)</h1>
                <div className='flex flex-col w-full border border-gray-300 rounded p-3 gap-3'>
                    <p className="font-bold">Upload File Excel (xlsx)</p>
                    <div className='flex flex-row flex-wrap gap-3 justify-between items-center'>
                        <div className='flex flex-row flex-wrap w-full md:w-fit gap-3 border border-gray-300 rounded px-2 py-2'>
                            <p>Upload File :</p>
                            <input type="file" />
                        </div>
